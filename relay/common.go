...
func shouldRetry(c *gin.Context, apiErr *types.OpenAIErrorWithStatusCode, channelType int) bool {
	...
	// Global and per-channel non-retryable filters
	nonRetryCodes, nonRetryKeywords := resolveNonRetryPolicy(c)

	// Status code based filter (e.g., 413, 422, 400 etc.)
	for _, sc := range nonRetryCodes {
		if apiErr.StatusCode == sc {
			return false
		}
	}

	// Error keyword based filter (case-insensitive search in message)
	msg := strings.ToLower(apiErr.OpenAIError.Message)
	if msg == "" {
		msg = strings.ToLower(apiErr.Message)
	}
	for _, kw := range nonRetryKeywords {
...
// resolveNonRetryPolicy merges global options with per-channel overrides from channel.CustomParameter
// Channel-level JSON schema examples (any one supported):
// 1) {"non_retry": {"status_codes":[400,413,422], "error_keywords":["invalid argument","too large"]}}
// 2) {"nonRetry":{"status_codes":[...], "error_keywords":[...]}}
func resolveNonRetryPolicy(c *gin.Context) ([]int, []string) {
	// start with global defaults/options
	codes := append([]int{}, config.NonRetryableStatusCodes...)
	keywords := append([]string{}, config.NonRetryableErrorKeywords...)

	// merge channel-level overrides if present
	chId := c.GetInt("channel_id")
	if chId <= 0 {
		return codes, keywords
	}

	ch, err := model.GetChannelById(chId)
	if err != nil || ch == nil {
		return codes, keywords
	}
	...
	nonRetry := extractMap(m, "non_retry")
	if nonRetry == nil {
		nonRetry = extractMap(m, "nonRetry")
	}
	if nonRetry == nil {
		return codes, keywords
	}
	...
	// status_codes
	if v, ok := nonRetry["status_codes"]; ok {
		if arr, ok := v.([]interface{}); ok {
			for _, it := range arr {
				switch t := it.(type) {
				case float64:
					codes = appendIfMissingInt(codes, int(t))
				case int:
					codes = appendIfMissingInt(codes, t)
				case string:
					if n, err := strconv.Atoi(strings.TrimSpace(t)); err == nil {
						codes = appendIfMissingInt(codes, n)
					}
				}
			}
		}
	...
	// error_keywords
	if v, ok := nonRetry["error_keywords"]; ok {
		switch vv := v.(type) {
		case []interface{}:
			for _, it := range vv {
				if s, ok := it.(string); ok {
					keywords = appendIfMissingStr(keywords, s)
				}
			}
		case []string:
			for _, s := range vv {
...
func appendIfMissingInt(arr []int, v int) []int {
	for _, x := range arr {
		if x == v {
			return arr
		}
...
func appendIfMissingStr(arr []string, v string) []string {
